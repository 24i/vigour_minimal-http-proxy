...
test('proxy request GET in path + cache buster', (t) => {
  t.plan(2)
  const server = createOrigin()
  const proxyServer = createProxyServer(8888)
  const req = http.request({
    ...
    port: 8888,
    method: 'GET',
    path: '/1.1.9PLOY_ENV%3Dstaging/proxy=' + encodeURIComponent('http://localhost:9090')
  }, (res) => {
    var data = ''
    var cnt = 0
    res.on('data', (chunk) => {
...
test('proxy request GET as a queryparameter enhance request handler, return a promise', (t) => {
  t.plan(2)
  const server = createOrigin()
  const proxyServer = createProxyServer(8888, false, (req) => {
    req.url = req.url.replace('something', 'http://localhost:9090')
